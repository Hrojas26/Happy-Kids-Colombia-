...
<p class="ps-3 text-center">Aquí podrás crear un bono para que los donantes lo reclamen y luego lo canjeen. <strong><br> ¡Anímate y genera TRÁFICO en tu empresa a través de Happy Kids Colombia!</strong></p>
...
<div class="container mt-5">
    <div class="row">
        ...
        <div class="col-md-6">
            <form action="{{ route('crear.bono') }}" method="POST" enctype="multipart/form-data" class="p-4 border rounded shadow-sm">
                @csrf
                <!-- Nombre del bono -->
                <div class="mb-3">
                    <label for="name" class="form-label fw-bold">Nombre del bono de descuento</label>
                    <input type="text" class="form-control" id="name" name="name" required oninput="updatePreview()" autocomplete="off">
                </div>

                <!-- Imagen del bono -->
                <div class="mb-3">
                    <label for="urlimage" class="form-label fw-bold">Imagen del bono</label>
                    <input type="file" class="form-control" id="urlimage" name="urlimage" required onchange="previewImage(event)" autocomplete="off">
                </div>

                <!-- Código de descuento -->
                <div class="mb-3">
                    <label for="codigoBono" class="form-label fw-bold">Código de descuento</label>
                    <p style="font-size:13px; font-style:italic;">Este código es exclusivo para que los donantes lo utilicen en tu tienda. Recuerda que es único, así que no lo compartas con nadie.</p>
                    <input class="form-control" id="codigoBono" name="codigoBono" required oninput="updatePreview()" autocomplete="off">
                </div>

                <!-- Descripción del bono -->
                <div class="mb-3">
                    <label for="description" class="form-label fw-bold">Descripción del bono</label>
                    <input class="form-control" id="description" name="description" required oninput="updatePreview()" autocomplete="off">
                </div>

                <!-- Fecha de expiración -->
                <div class="mb-3">
                    <label for="expirationDate" class="form-label fw-bold">Fecha de expiración del bono</label>
                    <input type="date" class="form-control" id="expirationDate" name="expirationDate" required oninput="updatePreview()" autocomplete="off">
                </div>

                <!-- Dirección de la empresa -->
                <div class="mb-3">
                    <label for="direccionEmpresa" class="form-label fw-bold">Dirección de la empresa</label>
                    <input type="text" class="form-control" id="direccionEmpresa" name="direccionEmpresa" required oninput="updatePreview()" autocomplete="off">
                </div>
...
<script>
    function updatePreview() {
        ...
        const expirationDate = document.getElementById('expirationDate').value ? `Fecha de expiración: ${document.getElementById('expirationDate').value}` : 'Fecha de expiración: ';
